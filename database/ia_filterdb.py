...
async def save_file(media):
    ...
    try:
        ...
        async def format_season_episode(html_caption,file_name = False):
            if html_caption:
                if not file_name:
...
async def search_db(query, offset,max =False):
    offset = int(offset)
    if max:
        max_results = 1000
    else:
    ...
    try:
        ...
        logging.exception(f'ERROR: #SERACH DB\n{e}')
        return [], None, 0
...
async def total_results_count(query):
    try:
        ...
        logging.exception(f'ERROR: #COUNT RESULT\n{e}')
        return None
...
async def send_filex(query_ep,user_id,client):
    ...
    match_filters = [
        {"$match": {"caption": {"$regex": pattern}}} for pattern in regex_patterns
    ]

    pipeline = match_filters + [
        {"$sort": {"created_at": -1}},  # Sorting by created_at field in descending order
        {"$limit": max_results}
    ]
